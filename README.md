...
> - **IDE:** Eclipse
> - **JDK:** v11.09
> - **SGBD:** MariaDB, **Nombre de la BD:** sgc
> - **SVR de aplicaciones:** Tomcat
> - **Otros:** jQuery, jQuery Validation Plugin, Bootstrap, Font Awesome, eclipselink, jstl, Jakarta.persistence

El desarrollo de la aplicación web se ha realizado siguiendo las recomendaciones y requerimientos planteados. Para tener una mejor organización se ha divido el modelo y controlador en 7 paquetes de acuerdo a su contenido: controller, dao, filter, entities, jpa, resources y test.

![imagen](resources/Imagen1.png)

En el paquete controller, se encuentran los Servlets que realizan las funciones CRUD y List para cada uno de los modelos que se pudieron abstraer durante el análisis de los requerimientos. En el paquete entities se encuentran las entidades de Java, los cuales son: Product, User, Company, Category, Bill Head y Bill Detail. Estos serán necesarios para trabajar con JPA.
Para la persistencia de datos, las interfaces y clases se encuentran distribuidas en los paquetes dao y jpa. Además, para poder manejar la seguridad y control de acceso a las funcionalidades se crearon filtros, los cuales se encuentran en el paquete filter. 
...
![imagen](resources/ImagenSesion.png)
...
Para ello, se crearon métodos CRUD para cada entidad abstraída de los requerimientos, en donde se persiste y se abstrae la información en base a consultas JPQL. Además, se desarrolló un método para realizar búsquedas específicas a través del Criteria.

![imagen](resources/Imagen29.png)

Para realizar consultas a la base de datos en donde se requiere más de una tabla, se creó el método: “findByJoin”. Este método recibe las clases (entidades de Java) a las que se quiere consultar, los atributos y valores requeridos, además de parámetros extras como los índices que se recuperen o mediante qué criterio se validan las consultas.

![imagen](resources/Imagen30.png)

A fin de evaluar el sistema desarrollado, se creó la clase “CreateTable” en el paquete de test, la cual inserta los datos necesarios en la BD, para realizar las pruebas correspondientes dentro de la aplicación web usando para ello JPA.
